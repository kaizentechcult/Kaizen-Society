...
export default function ProgressBar({ progress, color, textColor }: ProgressBarProps) {
  ...
  // Define default styles based on the theme
    ...
    <div className="w-full sm:w-auto">
      ...
      <div className="flex items-center gap-2 sm:gap-4">
        <div className={`w-full sm:w-32 md:w-48 h-2 ${bgColor} rounded-full overflow-hidden`}>
          {/* Render the motion div to show progress */}
          <motion.div
            initial={{ width: 0 }}
            animate={{ width: `${progress}%` }}
            transition={{ duration: 0.5 }}
            className={`${color || defaultColor} h-full`}
          />
        </div>
        {/* Display progress percentage */}
        <span
          className={`${textColor || defaultTextColor
            } text-xs sm:text-sm font-semibold whitespace-nowrap min-w-[40px] text-right`}
        >
          {progress}%
        </span>
      </div>
